...
def make_odf_certificate(minutes:'int', seconds:'int', player:'str', robot:'str'):
    ...
    robot_logo_href = document.addPicture('python/robocritters_logo.png')
    ...
    teletype.addTextToElement(p, f"during the {games_iteration}{make_ordinal(games_iteration)} Annual ")    
    ...
    # Save document
    file_name = f"./{player} with {robot}-robotcritter-certificate"
    document.save(file_name, True)
    return file_name

# send the certificate to the nearest printer and print it
def print_certificate(file_name: 'str'):
    pass
