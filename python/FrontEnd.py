...
class Frontend():
    ...
    def gui_slot_checked_in_entries(self) -> 'None':
        competition: 'str' = self.selected_competition.get()
    ...
    def gui_reset_competition_ring_assignments(self) -> 'None':
    ...
    def gui_make_odf_score_sheet(self) -> 'None':
    ...
    def gui_make_odf_certificates(self) -> 'None':
        competition: 'str' = self.selected_competition.get()
        event = self.events[competition]
        ...
        winners = [get_robot_entry_from_database(self.cursor, place1)]
        if len(place2) != 0:
            winners.append(
              get_robot_entry_from_database(self.cursor, place2))
        if len(place3) != 0:
            winners.append(
              get_robot_entry_from_database(self.cursor, place3))
