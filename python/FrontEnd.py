...
class Frontend():
    ...
    def __init__(self) -> 'None':
        ...
        with (connect(
          ...
          password=self.password,
            ...
            self.selected_competition.trace_add(
                "write", self.gui_competition_changed
            )
            ...
            tkinter.Button(
              ...
              command=lambda:
              self.gui_reset_competition_ring_assignments(self.cursor),
            ).pack()
    ...
    def gui_make_odf_certificates(self, cursor: Cursor) -> None:
        ...
        place1 = simpledialog.askstring(
          f"{competition}'s winners", "1st place Robot's ID?", )
...
def get_event_entries_from_database(
  ...
  event: Event,
    ...
    sql = "SELECT * FROM robots.robot WHERE competition = "\
...
def load_ring_assignments_from_database(
  ...
  event: Event,
    ...
    sql = "SELECT * FROM robots.\"ringAssignment\" "\
...
def reset_round_robin_tournaments(event: Event) -> list[str]:
    sql = []
    s = "DELETE from robots.\"ringAssignment\" " \
        + f"WHERE competition='{event.id}';"
    sql.append(s)

    # Clear all participated flags.
    s = "UPDATE robots.robot SET participated=FALSE " \
        + f"WHERE competition='{event.id}';"
    sql.append(s)
    ...
    if not event.round_robin_tournaments:
        ...
        for key in school_entries.keys():
            ...
            while len(entries):
                ...
                if not found:
                    event.round_robin_tournaments[
                      round_robin_keys[i % length]].add_entry(
                        entries[index])
        ...
        participation_query = \
            "UPDATE robots.robot SET participated=TRUE WHERE id in ("
    ...
    # There are already existing assignments.
        ...
        for tournament in event.round_robin_tournaments.values():
            for event_entry in tournament.event_entries:
                ...
                if remove:
                    ...
                    s = "DELETE from robots.\"ringAssignment\" WHERE " \
        ...
        for entry in slotted_entries:
            ...
            if add:
                ...
                s = "INSERT into robots.\"ringAssignment\" " \
                    "(robot, competition, ring, letter) " \
                    "VALUES " \
                    f"('{event_entry.entry.id}', '{event.id}', " \
                ...
                s = "UPDATE robots.robot SET participated=true " \
                    f"WHERE id = {event_entry.entry.id};"
