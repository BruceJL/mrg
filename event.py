...
class Event(object):
    ...
    def createRoundRobinTournaments(self):                
        ...
        for key, item in self.schoolEntries.items():
            ...
            while len(entries):
                ...
                self.ringEntries[i].append(entries[index])
    ...
    def printRingAssignments(self):
        for i in range(0, len(self.ringEntries)):
            print(self.competition + " Ring " + str(i+1))
            for j in range(0, len(self.ringEntries[i])):
                print("\t" + self.ringEntries[i][j].robotName + " : " + self.ringEntries[i][j].school)
